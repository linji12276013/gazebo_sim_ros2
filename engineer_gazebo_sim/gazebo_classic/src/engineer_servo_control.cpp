...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("engineer_servo_logger");
...
void publishCommands()
{
  ...
  if (count_ < 100)
  {
    ...
    msg->joint_names.emplace_back("link1_joint");
    msg->velocities.emplace_back(0.3);
...
int main(int argc, char** argv)
{
  ...
  node_options.arguments({"--ros-args", "-r", "__ns:=/engineer"});


  // This is false for now until we fix the QoS settings in moveit to enable intra process comms
  node_options.use_intra_process_comms(false);
  node_ = std::make_shared<rclcpp::Node>("engineer_servo_node", node_options);
  ...
  // auto rml_options = std::make_shared<robot_model_loader::RobotModelLoader::Options>("urdf", "srdf");
  auto planning_scene_monitor = std::make_shared<planning_scene_monitor::PlanningSceneMonitor>(
      node_, "robot_description", "planning_scene_monitor");

  // auto robot_model_loader_ = std::make_shared<robot_model_loader::RobotModelLoader>(node_, "robot_description");


  // Here we make sure the planning_scene_monitor is updating in real time from the joint states topic
  if (planning_scene_monitor->getPlanningScene())
  {
    RCLCPP_INFO(LOGGER, "Planning Scene Get!");
    planning_scene_monitor->startStateMonitor("/engineer/joint_states");
    planning_scene_monitor->setPlanningScenePublishingFrequency(25);
    planning_scene_monitor->startPublishingPlanningScene(planning_scene_monitor::PlanningSceneMonitor::UPDATE_SCENE,
                                                         "/engineer/moveit_servo/publish_planning_scene");
